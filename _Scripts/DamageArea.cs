...
public partial class DamageArea : Area2D
{
	...
	private void DamageTarget(Node2D target)
	{
		...
		GD.Print((target != GetParent().GetParent()) + " " + (target.FindChild("Health") is Health));
		if (target != GetParent().GetParent() && target.FindChild("Health") is Health)
		{
			Health bodyHealth = target.FindChild("Health") as Health;
			bodyHealth.TakeDamage(1);
		}
	}
